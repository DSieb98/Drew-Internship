...
| D-28 | M3 reactivated for Instantly.ai email automation (REQ-05/06) — D-03/D-04's "deferred indefinitely" was a status note, not a rescinded decision, so this resumes deferred work rather than overriding a locked one (Drew's call, D-14 authority). This reclaimed the M3 folder from D-27's nurture engine, which had no other number reserved for it. Rather than delete real, shipped, live-verified code's documentation (`NurturePage.tsx`, `nurtureTouches`, 4 live LACRM custom fields — D-27/a/b/c), the nurture-engine spec moved to `docs/specs/M7/` (next open number) with references updated; no code changed. See `docs/specs/M3/M3-00-index.md`'s reactivation note and `docs/specs/M7/M7-00-index.md`'s numbering note. | M3 reactivation |

...
**Goal:** Build the monthly-summary reporting view in SalesForge, showing whatever metrics T01 confirmed have a real data source.
...
- A dashboard page/section presenting confirmed metrics as a monthly summary, matching the plain-language, JAWS-first UX pattern already established elsewhere in SalesForge (Today/All Leads pages).
...
Layout, component structure, and specific visual/interaction design are Claude Code's call within PRINCIPLE-03's constraints and the plain-language tone already established in the rest of SalesForge. This replaces `src/pages/ReportsPage.tsx`'s M0 "coming later" placeholder.

...
- Two-way sync of lead records: creates, updates, and stage changes made in SalesWhiz write to LACRM; changes in LACRM (or by other reps) are reflected back in SalesWhiz.
- REQ-09 stage tracking using LACRM's confirmed stage names (from T01), not the prototype-carried placeholder list.
- Handling for a lead that exists in LACRM but was never imported through SalesWhiz (M0-T02), and vice versa, per the resolution procedure already defined in spec v1.2 §5 for reconciling stage-name mismatches.

## Out of scope

- Nurture state, hot-alert status, call history, notes, and score sync — T04.
- Conflict resolution mechanics — T05 (this task assumes the happy path; T05 handles what happens when SalesWhiz and LACRM disagree).
...
- Creating or editing a lead in SalesWhiz is reflected in LACRM.
- A stage change in SalesWhiz writes the correct LACRM stage name.
- A change made directly in LACRM appears in SalesWhiz on next sync/load.
...
SalesWhiz writes the correct LACRM stage name" needs something in SalesWhiz that can actually
...
apps, not SalesWhiz — but T03's acceptance criteria requires the write path to be exercisable, so

...
- A **hard stop**: once consumption crosses a configurable ceiling (recommend defaulting to 90 credits — leaving a buffer below the 100-credit free-tier cap), no further enrichment calls fire until the next billing cycle. Leads that would have been enriched instead get flagged (feeds into T04's "enrichment pending" state) rather than silently skipped.
- A **warning threshold** below the hard stop (recommend 70 credits) that notifies Drew — email or a Make.com notification, whichever is lower-effort — so there's lead time before the hard stop engages, not just a stop with no warning.
...
- Drew has reviewed and confirmed the default thresholds (90 hard stop / 70 warning) or set his own.

## How — Claude Code decides

Not applicable — this is Make.com configuration, not SalesForge repo code. If any SalesForge-side surfacing of "enrichment paused — free tier limit reached" is needed (vs. per-lead "pending"), that's covered under T04, not here.

...
**Owner:** Drew (Make.com retry logic) + Claude Code (SynetheixSales-side "enrichment pending" state).
...
- **SynetheixSales side (Claude Code):** an "enrichment pending" indicator on a lead (in the lead card and/or drawer) so Tim can see that a lead's score is based on fallback data, not a silent gap. This must use the accessibility patterns already established (aria-live for state changes, not a silent visual-only badge) per PRINCIPLE-03.
...
- PRINCIPLE-03: any new pending/retry state shown in SynetheixSales follows the existing live-region/announcement pattern — never a silent icon-only indicator.
...
- A lead blocked by T00a's hard stop shows the same "pending" treatment in SynetheixSales as a retry-queued lead, distinguishable by reason if feasible.
...
The specific SynetheixSales-side component/indicator design (badge placement, aria-label wording, whether it's surfaced in the list view or only the drawer) is Claude Code's call within PRINCIPLE-03's constraints.

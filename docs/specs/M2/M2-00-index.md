...
- **M1 is functionally complete** — specifically M1-T01 (LACRM field mapping) and M1-T03 (lead record sync), since enriched fields need a defined path into LACRM (PRINCIPLE-01: LACRM is the source of truth, not SalesWhiz's in-memory or synced-but-unmapped state).
...
| :-- | :-- | :-- | :-- | :-- |
| **T00** | Clay.com account & enrichment field schema | Which fields Clay returns and how they map to SalesWhiz/LACRM concepts | — | Drew (+ Clay.com config) |
...
| **T02** | LACRM enrichment write-back | Enriched fields land in LACRM as appended data, not overwritten raw import data | T00, T01, M1-T01, M1-T03 | Drew (Make.com) + Claude Code (if SalesWhiz-side read logic needed) |
...
| **T04** | Enrichment failure handling & visibility | Failed/pending enrichment flagged, queued for retry, pipeline continues; SalesWhiz shows an honest "enrichment pending" state rather than silently scoring on stale/proxy data | T01–T03 | Drew (Make.com retry logic) + Claude Code (SalesWhiz-side state) |

**Note on task ownership:** T00 and T01 are primarily Drew's own configuration work in Clay.com and Make.com — no-code tools outside the SalesWhiz repo. Claude Code's role there, if any, is limited to reviewing the field schema against what the scoring engine needs (T00) and to documentation. T02 is a boundary task — whichever side (Make.com scenario vs. SalesWhiz code) ends up owning the write depends on decisions already made in M1; the task file below flags this rather than presupposing it. T03 and T04's SalesWhiz-side pieces are the tasks that actually get handed to Claude Code as code work.
